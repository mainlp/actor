...
from bert.modeling import BertForSequenceClassification, BertConfig, MultiTaskBert
from bert.tokenization import BertTokenizer
from bert.optimization import BertAdam, WarmupLinearSchedule

from loader import GabProcessor, WSProcessor, NytProcessor, convert_examples_to_features, GabAnnotatorProcessor, GabMajorityProcessor
...
from mlflow import log_metric, log_param, log_artifact, log_params
import mlflow
...
def acc_and_f1(preds, labels, pred_probs, task_name):
...
def compute_metrics(task_name, preds, labels, pred_probs):
    ...
    return acc_and_f1(preds, labels, pred_probs, task_name)
...
def main():
    ...
    parser.add_argument("--load_dir", default=None, type=str, help="The directory to load model from")
    ...
    if args.do_train:
        ...
        if task_name == 'gab_anno':
            model = MultiTaskBert.from_pretrained(args.load_dir, num_labels=num_labels)
        else:
    ...
    if args.do_train:
        ...
        if task_name == 'gab_anno':
            train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_annotator_ids)
        else:
...
def validate(args, model, processor, tokenizer, output_mode, label_list, device, num_labels,
    ...
    if task_name == 'gab_anno' and args.eval_mode == 'annotator':
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if task_name != 'gab_anno':
            ...
            loss_fct = CrossEntropyLoss()
            if args.eval_mode == 'annotator':
                input_ids, input_mask, segment_ids, label_ids, annotator_ids = batch
                
                with torch.no_grad():
                    tmp_eval_loss, preds_anno = model(input_ids, segment_ids, input_mask, labels=label_ids, annotator_ids=annotator_ids, loss_fct=loss_fct, pred=True, pre_mode=args.eval_mode)
            else:
                input_ids, input_mask, segment_ids, label_ids = batch
                with torch.no_grad():
